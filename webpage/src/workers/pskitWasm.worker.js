...
const handlers = {
  ...
  annotate_binding_pairs: (wasm, msg) => {
    ...
    try {
      ...
      return {
        payload: { ok: true, kind: "binding_pairs", pairs, distances },
        transfer: [],
      };
    } finally {
      try {
  ...
  d_map: (wasm, msg) => {
    ...
    try {
      ...
      if (rawValues instanceof Float64Array) values = rawValues;
        ...
        values = Float64Array.from(rawValues.map((x) => Number(x)));
      else
        values = Float64Array.from(
          Array.from(rawValues || []).map((x) => Number(x)),
        );

      return {
        payload: { ok: true, kind: "contact_map", axis, values },
