...
function getPskitWorker() {
  ...
  _pskitWorker = new Worker(
    new URL("../workers/pskitWasm.worker.js", import.meta.url),
    { type: "module" },
  );
...
export async function splitByChainInWorker(bytes, format) {
  ...
  return await callPskitWorker(
    { fn: "split_by_chain", bytes, format },
    [bytes.buffer],
    { timeoutMs: 120000 },
  );
}

export async function extractFragmentInWorker(
  bytes,
  chain_id,
  start,
  end,
...
export async function annotateBindingPairsInWorker(bytes, cutoff, format) {
  ...
  return await callPskitWorker(
    { fn: "annotate_binding_pairs", bytes, cutoff, format },
    [bytes.buffer],
    { timeoutMs: 120000 },
  );
}
...
export async function fetchRcsbFile(id) {
  ...
  const tryUrls = [
    {
      url: `https://files.rcsb.org/download/${upper}.cif`,
      format: "cif",
      source: `${upper}.cif`,
    },
    {
...
export function chunksToDownloadItems({ chunks, base, format, source }) {
  ...
  for (const key of keys) {
    ...
    out.push(
      bytesToDownloadItem({ bytes: data, filename, source, key: String(key) }),
    );
  }
...
export async function downloadGroupedAsZip(grouped, zipName = "results.zip") {
  ...
  for (const group of grouped || []) {
    const folderName =
      sanitizeKey(stripExtension(group.source || "results")) || "results";
